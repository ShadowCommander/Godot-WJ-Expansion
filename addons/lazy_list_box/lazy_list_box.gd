...
func _create_item_pool():
	...
	if item_template:
		for i in pool_size:
			...
			if item is LazyListEntry:
				item.internal_button.focus_entered.connect(_on_item_focus_entered.bind(item))
				item.internal_button.focus_exited.connect(_on_item_focus_exited.bind(item))
			item.focus_entered.connect(_on_item_focus_entered.bind(item))
			item.focus_exited.connect(_on_item_focus_exited.bind(item))
			item.entry_button_down.connect(_on_entry_button_down)
			item.entry_button_up.connect(_on_entry_button_up)
			item.entry_pressed.connect(_on_entry_pressed)
			item.entry_toggled.connect(_on_entry_toggled)
...
#region Button selecton handling
...
func _on_entry_pressed(index: int, data) -> void:
	entry_pressed.emit(index, data)

func _on_entry_button_down(index: int, data) -> void:
	entry_button_down.emit(index, data)

func _on_entry_button_up(index: int, data) -> void:
	entry_button_up.emit(index, data)

func _on_entry_toggled(index: int, data, toggled_on: bool) -> void:
	entry_toggled.emit(index, data, toggled_on)
